    ...
    return (
        <div className="elevated-card p-6 animate-fade-in-up" style={{ animationDelay: '300ms' }}>
            <div className="flex items-center gap-3 mb-4">
                ...
                {alerts.length > 0 && (
                    <span className="text-xs px-2 py-0.5 rounded-md" style={{ background: 'rgba(255,77,109,0.12)', color: 'var(--accent-coral)' }}>
                        {alerts.length}
                    </span>
                )}
            </div>
            <div className="flex flex-col gap-2.5 max-h-[340px] overflow-y-auto pr-1">
                {alerts.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-8 gap-2">
                        ...
                        <p style={{ color: 'var(--accent-green)', fontSize: 13, fontFamily: 'Syne, sans-serif', fontWeight: 600 }}>All clear!</p>
                        <p style={{ color: 'var(--text-muted)', fontSize: 12 }}>No urgent alerts right now</p>
                    </div>
                ) : (
                    alerts.map((alert, i) => {
                        ...
                        return (
                            ...
                            >
                                ...
                                <div className="flex items-center justify-between gap-2 flex-wrap pl-2">
                                    <span className="font-semibold text-sm" style={{ fontFamily: 'Syne, sans-serif', color: 'var(--text-primary)' }}>
                                        {alert.client}
                                    </span>
                                ...
                                {isSubstantive(alert.detail) && (
                                    <p className="text-xs leading-relaxed line-clamp-2 pl-2" style={{ color: 'var(--text-secondary)' }}>
